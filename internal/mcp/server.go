...
// handleClaudeMdLint checks CLAUDE.md health and returns lint results
func (s *Server) handleClaudeMdLint(args map[string]interface{}) (interface{}, error) {
	...
	// Count rules files
	rulesCount := s.countRulesFiles()

	// Recommendations
	if len(issues) > 0 {
		recommendations = append(recommendations, "Move large sections to .claude/rules/ modules")
		recommendations = append(recommendations, "Keep CLAUDE.md focused on overview and key decisions")
		recommendations = append(recommendations, "Use 'tk claudemd stats' to see full breakdown")
	}
...
// handleClaudeMdStats returns detailed section breakdown of CLAUDE.md
func (s *Server) handleClaudeMdStats(args map[string]interface{}) (interface{}, error) {
	filePath, _ := args["file"].(string)
	if filePath == "" {
		filePath = "CLAUDE.md"
	}
...
// analyzeClaudeMdFile parses CLAUDE.md and returns line count and sections
func (s *Server) analyzeClaudeMdFile(path string) (int, []claudeMdSection, error) {
	...
	var currentSection *claudeMdSection
	...
	for scanner.Scan() {
		...
		if matches := h2Pattern.FindStringSubmatch(line); len(matches) > 1 {
			...
			currentSection = &claudeMdSection{
				name:      matches[1],
				startLine: lineNum,
			}
		}
...
func (s *Server) countRulesFiles() int {
	count := 0
	rulesDir := ".claude/rules"
	if entries, err := os.ReadDir(rulesDir); err == nil {
		for _, entry := range entries {
...
func (s *Server) countFileLines(path string) int {
	file, err := os.Open(path)
	if err != nil {
		return 0
	}

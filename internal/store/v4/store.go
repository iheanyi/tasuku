...
// =============================================================================
// Granular Access Operations
// =============================================================================

// readIndex reads and parses the index file.
...
// ListTasks returns a filtered list of tasks using the index.
func (s *Store) ListTasks(filter task.TaskFilter) ([]task.TaskMeta, error) {
	idx, err := s.readIndex()
	if err != nil {
		return nil, err
	}
	...
	for id, meta := range idx.Tasks {
		// Apply filters
		if filter.Status != nil && task.Status(meta.Status) != *filter.Status {
			continue
		}
		if filter.Tag != nil {
			found := false
		...
		if filter.Owner != nil {
			// Note: Owner is in meta as string (empty if nil)
			if meta.Owner != *filter.Owner {
				continue
			}
		}

		// Convert to task.TaskMeta
		// Note: Description is not in index, so we leave it empty for now.
		// If description is needed, the caller might need to fetch it or we add it to index.
		// For `tk list`, descriptions ARE needed. This is a trade-off.
		// For now, we'll return what we have.
		// Optimization TODO: Add description/title to index.
		...
		results = append(results, task.TaskMeta{
			...
			Tags:        meta.Tags,
			// Description: "", // Missing in index!
		})
	}
	
	// If description is missing, we must read tasks for now until index is upgraded.
	// However, to satisfy the interface we return what we have.
	// The CLI/MCP will likely need descriptions. 
	// For V1 of this feature, let's read the task file to get description if it's not in index.
	// This makes it slower than ideal but still faster than reading everything if we filter first.
	// But wait, iterate map is random order.
	
	// Let's implement the "read for description" fallback for now.
	for i := range results {
		t, _, err := s.readTask(results[i].ID)
		if err == nil {
...
func (s *Store) UpdateTask(id string, fn func(*task.Task) error) error {
